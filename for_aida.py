...
directory = 'data/spectra/' # e.g., mwmStar-0.6.0-114879184.fits
...
test_labels_arr, true_labels_arr = loocv.loocv(df, wl, flux_tr, ivar_tr, label_names)
...
print(preds)
preds.to_csv(path+'data/preds_dnu.csv', index=False)
...
plt.xlim([-0.6, 0.5])
plt.ylim([-0.6, 0.5])
plt.savefig(path+'plots/mg_h_check.png')
plt.show()
...
plt.xlim([0, 14])
plt.ylim([0, 14])
plt.savefig(path+'plots/age_check.png')
plt.show()
...
plt.xlim([0, 160])
plt.ylim([0, 160])
plt.savefig(path+'plots/Dnu_check.png')
plt.show()