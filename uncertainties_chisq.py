...
chisq_dones_old = pd.read_csv(path+'data/chisq_dones_old.csv')
aspcap_dones_old = aspcap_df.loc[aspcap_df['aspcap_sdss_id'].isin(chisq_dones_old['sdss_id'])]
aspcap_dones = pd.concat([aspcap_dones, aspcap_dones_old])
### i need to get these two files straight. are there really repeats? why are there so few qualifying stars? 
